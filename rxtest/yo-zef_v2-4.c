...
//			LED1=OFF;
		...
		if((goal_frag1==1 && max_frag==1 && (ave_SEN[2] < thre_max && ave_SEN[3] < thre_max && ave_SEN[4] < thre_max && ave_SEN[5] < thre_max && ave_SEN[6] < thre_max))){
//			LED1=ON;
			ERR=max_ERR;
		}
				
		if((goal_frag1==1 && min_frag==1 && (ave_SEN[1] < thre_max && ave_SEN[2] < thre_max && ave_SEN[3] < thre_max && ave_SEN[4] < thre_max && ave_SEN[5] < thre_max))){
//			LED1=ON;
			ERR=min_ERR;
		}
		...
		if(circl_count==1){	
			if((ave_SEN[corner] > SEN_thre[corner]) && goal_frag2==1 && curve_frag==0){
				...
				if(angl_vel >= 0){
					...
					ENC_deg[ENC_count]=(double)angl_vel*0.00639919;
				}	
				else if(angl_vel < 0){
					angl_vel*=(-1);
					curvature=(RIGHT_ENC+LEFT_ENC)*Tred/(2*angl_vel);
...
//		dec_out((short)sub_SEN_check[i], 8); outs(" ");
	//	dec_out(vel_check[i], 8); outs(" ");
		...
		dec_out((short)ENC_deg[i], 8); outs(" ");
