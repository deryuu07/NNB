...
void mot_drive(double mot_in_p, double mot_in_p_sub, double mot_in_d, double mot_in_i, double mot_in_g, double mot_in_speed_R, double mot_in_sum_speed_R, double mot_in_speed_L, double mot_in_sum_speed_L);
void Battery_Check();
void countdown();
void trace();
void out_senc();
...
int calc_senc()
{
	...
	if(start0==1){
		...
		if((max==ave_SEN[3] || max==ave_SEN[4]) || 
		   ((ave_SEN[1] < 30.0) && (ave_SEN[2] < 30.0) && (ave_SEN[5] < 30.0) && (ave_SEN[6] < 30.0))){
			ERR=ave_SEN[3]-ave_SEN[4];	//○○● ●○○
			LED1=OFF;
			K_P=first_K_P;
		}
		else if((max==ave_SEN[2]) && (ave_SEN[2] > 30.0) && goal_frag2==1){
			ERR=(ave_SEN[2]-ave_SEN[4]);	//○●○ ●○○
			LED1=OFF;
			K_P=first_K_P*2.1;
		}
		else if((max==ave_SEN[5]) && (ave_SEN[5] > 30.0) && goal_frag2==1){
			ERR=(ave_SEN[3]-ave_SEN[5]);	//○○● ○●○
			LED1=OFF;
			K_P=first_K_P*2.1;
		}			
		else if(max==ave_SEN[1] && (ave_SEN[1] > 30.0) && goal_frag2==1){
			ERR=(ave_SEN[1]-ave_SEN[4]);	//●○○ ●○○
			LED1=OFF;
			K_P=first_K_P*2.5;
		}
		else if(max==ave_SEN[6] && (ave_SEN[6] > 30.0) && goal_frag2==1){
...
void stop_manager()		//停止処理
{
	while(stop_count!=200){
		mot_drive(ERR, sub_ERR, PRE_ERR, SUM_ERR, GYRO_ERR, SPEED_ERR_R, SPEED_SUM_ERR_R, SPEED_ERR_L, SPEED_SUM_ERR_L);
	}
...
short goal_check(long RIGHT_ENC, long LEFT_ENC)
{
	...
	if(first_check==2){		
		if((ave_SEN[1] > SEN_thre[1] || ave_SEN[2] > SEN_thre[2]) && (ave_SEN[5] > SEN_thre[5] || ave_SEN[6] > SEN_thre[6]) &&goal_frag2==1){
			goal_frag2=0;
		}
...
void TIMER_CALL()
{
	...
	if(start0==1){				
		...
		if(ave_SEN[9]-ave_SEN[10] > 5.0 || ave_SEN[9]-ave_SEN[10] < -5.0)
			ST_curve_check++;
		else if(ave_SEN[9]-ave_SEN[10] < 5.0 && ave_SEN[9]-ave_SEN[10] > 0.0){
			ST_curve_check--;
			if(ST_curve_check<0)
				ST_curve_check=0;
		}
		...
		if(EncTimeCount==50){
			EncTimeCount=0;
//			vel_check[vel_count]=(short)now_vel_R;
//			vel_count++;
