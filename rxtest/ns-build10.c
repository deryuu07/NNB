...
void make_shortcut3()
{
	...
	for(i=0; i<200; i++){
		if(abs((course_data[i]) > 1000) && ((ENC_dis[i]*mmpp) > 800.0)){
			for(j=i; j<200; j++){
				if((abs(course_data[j+1]) > 1000) && ((ENC_dis[j+1]*mmpp) > 800.0)){
					ENC_dis[i]+=ENC_dis[j+1];
					course_data[i]=(course_data[i]+course_data[i+1])/2;
					st_count++;
				}
				else{
					if(st_count>1){
	...
	if(tmp > 300.0){
		if(tmp > 2.0*accel_dis[0]){
			...
			if(tmp > 2.0*accel_dis[1]){
				now_comp=1;
				if((tmp > 2.0*accel_dis[2]) && (now_R > 3000))
					now_comp=2;
			}
		}
	}
...
void calc_next_speed(short next_R)
{
	if(ENC_dis[ENC_count+1]*mmpp > 300.0){
		if(next_R < 150) 					next_top_speed=curve_speed_para[R10];
		if(next_R >= 150 && next_R < 200) 	next_top_speed=curve_speed_para[R15];
		if(next_R >= 200 && next_R < 250){	next_top_speed=curve_speed_para[R20];}
		if(next_R >= 250 && next_R < 300) 	next_top_speed=curve_speed_para[R25];
		if(next_R >= 300 && next_R < 350) 	next_top_speed=curve_speed_para[R30];
		if(next_R >= 350 && next_R < 400) 	next_top_speed=curve_speed_para[R35];
		if(next_R >= 400 && next_R < 450) 	next_top_speed=curve_speed_para[R40];
		if(next_R >= 450 && next_R < 500) 	next_top_speed=curve_speed_para[R45];
		if(next_R >= 500 && next_R < 550){	next_top_speed=curve_speed_para[R50];}
		if(next_R >= 550 && next_R < 600) 	next_top_speed=curve_speed_para[R55];
		if(next_R >= 600 && next_R < 650) 	next_top_speed=curve_speed_para[R60];
		if(next_R >= 650 && next_R < 700) 	next_top_speed=curve_speed_para[R65];
		if(next_R >= 700 && next_R < 750) 	next_top_speed=curve_speed_para[R70];
		if(next_R >= 750 && next_R < 800) 	next_top_speed=curve_speed_para[R75];
		if(next_R >= 800 && next_R < 850){	next_top_speed=curve_speed_para[R80];}
		if(next_R >= 850 && next_R < 900) 	next_top_speed=curve_speed_para[R85];
		if(next_R >= 900 && next_R < 950) 	next_top_speed=curve_speed_para[R90];
		if(next_R >= 950 && next_R < 1000) 	next_top_speed=curve_speed_para[R95];
	}
	else{
		next_top_speed=curve_speed_para[R10];
	}
}

void calc_now_speed(short now_R, float first_speed)
{
	if(ENC_dis[ENC_count]*mmpp > 300.0){
		if(now_R < 150) 					now_top_speed=curve_speed_para[R10];
		if(now_R >= 150 && now_R < 200) 	now_top_speed=curve_speed_para[R15];
		if(now_R >= 200 && now_R < 250){	now_top_speed=curve_speed_para[R20];}
		if(now_R >= 250 && now_R < 300) 	now_top_speed=curve_speed_para[R25];
		if(now_R >= 300 && now_R < 350) 	now_top_speed=curve_speed_para[R30];
		if(now_R >= 350 && now_R < 400) 	now_top_speed=curve_speed_para[R35];
		if(now_R >= 400 && now_R < 450) 	now_top_speed=curve_speed_para[R40];
		if(now_R >= 450 && now_R < 500) 	now_top_speed=curve_speed_para[R45];
		if(now_R >= 500 && now_R < 550){	now_top_speed=curve_speed_para[R50];}
		if(now_R >= 550 && now_R < 600) 	now_top_speed=curve_speed_para[R55];
		if(now_R >= 600 && now_R < 650) 	now_top_speed=curve_speed_para[R60];
		if(now_R >= 650 && now_R < 700) 	now_top_speed=curve_speed_para[R65];
		if(now_R >= 700 && now_R < 750) 	now_top_speed=curve_speed_para[R70];
		if(now_R >= 750 && now_R < 800) 	now_top_speed=curve_speed_para[R75];
		if(now_R >= 800 && now_R < 850){	now_top_speed=curve_speed_para[R80];}
		if(now_R >= 850 && now_R < 900) 	now_top_speed=curve_speed_para[R85];
		if(now_R >= 900 && now_R < 950) 	now_top_speed=curve_speed_para[R90];
		if(now_R >= 950 && now_R < 1000) 	now_top_speed=curve_speed_para[R95];
		if(now_R >= 1000){ 	
			calc_accel_dis(first_speed);
			Accel_manager(ENC_count, now_R);
			now_top_speed=speed_para[now_comp];
		}
	}
	else{
		now_top_speed=curve_speed_para[R10];
	}
}
...
//		if(((ave_SEN[corner2]>SEN_thre[corner2])) && corner_frag==0 && goal_frag2==1 && (distance2 > 50.0)){			
		...
		if(goal_count==circle_number){
			...
			course_data[ENC_count+1]=0;
			course_data[ENC_count+2]=0;
			ENC_dis[ENC_count]=distance;
		...
		if(accel_safety==0){
			if(goal_frag1==1){	
				if(corner_frag==0){
					...
					/***加減速処理***/
					if((accel_distance > distance2) && (next_R!=0)){
						LED1=ON;
						Desire_vel+=(Accel[now_comp]/1000.0);
						if(Desire_vel > now_top_speed)
							Desire_vel=now_top_speed;
					...
					if(accel_distance < distance2 && distance2 < (now_comp_distance-accel_distance)){
						LED2=ON;
						Desire_vel=now_top_speed;
						calc_speed(Desire_vel);	
					}  
					if(distance2 > (now_comp_distance-accel_distance)){
						LED3=ON;
						if(now_top_speed > next_top_speed){
							Desire_vel-=(Accel[now_comp]/1000.0);
							if(Desire_vel < next_top_speed)
								Desire_vel=next_top_speed;
						}
						else{
							Desire_vel-=(Accel[now_comp]/1000.0);
...
//					sub_ERR=0.0;
					...
					K_D=first_K_D;
					K_P_GYRO=first_K_G;
//					now_top_speed=Desire_next_speed;
					ENC_count++;
					now_R=abs(course_data[ENC_count]);
					next_R=abs(course_data[ENC_count+1]);
					first_speed=now_vel;
					calc_now_speed(now_R, first_speed);
					calc_next_speed(next_R);
					accel_distance=((now_top_speed*now_top_speed)-(first_speed*first_speed))/(2.0*Accel[2]);	
					if(accel_distance < 0.0) accel_distance*=(-1.0);
...
void trace2()	//二週目以降
{
	...
	ENC_count=0;
	calc_now_speed(abs(course_data[0]), Desire_vel_nomal);
...
void init_thre(){
	...
	if(debug_mode_flag==0){
		SEN_thre[goal]  =180;//2700;//max_thre*0.3;//TargetGoal*K_goal;//max_thre*0.3;//90;
		SEN_thre[corner]=220;//160;//1200;//max_thre*0.3;//60;
