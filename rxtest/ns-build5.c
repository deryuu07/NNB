...
{	
	...
	if(tmp > 2.2*sub_accel_dis[0]){
		now_comp=0;
		if(tmp > 2.2*sub_accel_dis[1]){
			now_comp=1;
			if((tmp > 2.2*sub_accel_dis[2]) && (now_R > 3000)){
				now_comp=2;
			}
		}
	}
...
{	
	accel_dis=((now_max_speed*now_max_speed)-(Initial_velocity*Initial_velocity))/(2.0*Accel[2]);
	if(accel_dis < 0.0) accel_dis*=(-1.0);
}

void sub_calc_accel_dis(float Initial_velocity)
{	
	sub_accel_dis[0]=((speed_para[0]*speed_para[0])-(Initial_velocity*Initial_velocity))/(2.0*Accel[2]);
	sub_accel_dis[1]=((speed_para[1]*speed_para[1])-(Initial_velocity*Initial_velocity))/(2.0*Accel[2]);
	sub_accel_dis[2]=((speed_para[2]*speed_para[2])-(Initial_velocity*Initial_velocity))/(2.0*Accel[2]);	

//	if(sub_accel_dis[0] < 0.0) sub_accel_dis[0]*=(-1.0);
//	if(sub_accel_dis[1] < 0.0) sub_accel_dis[1]*=(-1.0);
//	if(sub_accel_dis[2] < 0.0) sub_accel_dis[2]*=(-1.0);
}
...
//		}
	...
	/********************コーナー処理*******************************/	
		if(((ave_SEN[corner2]>SEN_thre[corner2]) || (ave_SEN[corner]>SEN_thre[corner])) && corner_frag==0 && goal_frag2==1 && (distance2 > 50.0)){
...
void goal_check2(long RIGHT_ENC, long LEFT_ENC, short err)	//カーブ判定
{
	...
	short now_R=course_data[ENC_count];
	short next_R=course_data[ENC_count+1];
	...
	/********************コーナー処理*******************************/	
		if(accel_safety==0){
			if(goal_frag1==1){	
				if(corner_frag==0){
					if(now_max_speed > next_max_speed){
						if((accel_dis > distance2) && (next_R!=0)){
							LED1=ON;
							K_P=first_K_P*0.9;
							Desire_vel+=(Accel[2]/1000.0);
							if(Desire_vel > now_max_speed){
								Desire_vel=now_max_speed;
							}
							calc_speed(Desire_vel);	
						}
						if(accel_dis < distance2 && distance2 < (now_comp_distance-accel_dis)){
							LED2=ON;
							K_P=first_K_P*0.2;
//							K_D=first_K_D*2.5;
							Desire_vel=now_max_speed;
						...
						if(distance2 > (now_comp_distance-accel_dis)){
							LED3=ON;
							K_P=first_K_P*0.9;
							Desire_vel-=(Accel[2]/1000.0);
							if(Desire_vel < next_max_speed){
								Desire_vel=next_max_speed;
							}
							calc_speed(Desire_vel);	
						}					
					}
...
//									K_D=first_K_D*2.5;
								...
								if(distance2 > (now_comp_distance-accel_dis)){
									...
									K_P=first_K_P*0.9;
									Desire_vel-=(Accel[2]/1000.0);
									if(Desire_vel < ((speed_para[0]+speed_para[1])/2.0)){
										Desire_vel=(speed_para[0]+speed_para[1])/2.0;
									}
									calc_speed(Desire_vel);	
								}
							}
...
//					calc_speed(Desire_vel);	
	//				sub_ERR=0.0;
					...
					now_max_speed=next_max_speed;
					first_speed=now_vel;
					calc_accel_dis(first_speed);
					if(next_R > 2000){
						if(now_R > 2000){
							sub_calc_accel_dis((speed_para[0]+speed_para[1])/2.0);
							Accel_manager(ENC_count, now_R);
							next_max_speed=speed_para[now_comp];
						}
						else{
							sub_calc_accel_dis(now_max_speed);
							Accel_manager(ENC_count, now_R);
							next_max_speed=speed_para[now_comp];
						}
					}
...
//							LED3=OFF;
		...
		/*********************ゴール処理*******************/				
		if((ave_SEN[goal] > SEN_thre[goal]) && goal_frag2==1 && (course_data[ENC_count+5]==0 || accel_safety==1) ){
			goal_count++;
			goal_frag2=0;
		}
		if((ave_SEN[goal] > SEN_thre[goal]) && goal_frag2==1 && accel_safety==1){
			goal_count++;
			goal_frag2=0;
		}
...
void select_speed()
{
	...
	while(select_frag==0){
		...
		if(ave_SEN[1] > 180.0){
			...
			curve_speed_para[R20]=(Desire_vel_nomal*a)+20.0;
			curve_speed_para[R25]=(Desire_vel_nomal*a)+40.0;
			curve_speed_para[R30]=(Desire_vel_nomal*a)+60.0;
			curve_speed_para[R35]=(Desire_vel_nomal*a)+80.0;
			curve_speed_para[R40]=(Desire_vel_nomal*a)+100.0;
			curve_speed_para[R45]=(Desire_vel_nomal*a)+120.0;
			curve_speed_para[R50]=(Desire_vel_nomal*a)+140.0;
			curve_speed_para[R55]=(Desire_vel_nomal*a)+160.0;
			curve_speed_para[R60]=(Desire_vel_nomal*a)+180.0;
			curve_speed_para[R65]=(Desire_vel_nomal*a)+200.0;
			curve_speed_para[R70]=(Desire_vel_nomal*a)+220.0;
			curve_speed_para[R75]=(Desire_vel_nomal*a)+240.0;
			curve_speed_para[R80]=(Desire_vel_nomal*a)+260.0;
			curve_speed_para[R85]=(Desire_vel_nomal*a)+280.0;
			curve_speed_para[R90]=(Desire_vel_nomal*a)+300.0;
			curve_speed_para[R95]=(Desire_vel_nomal*a)+320.0;
			curve_speed_para[R100]=(Desire_vel_nomal*a)+320.0;		//2000mm/sくらい
			curve_speed_para[R115]=(Desire_vel_nomal*a)+350.0;
			curve_speed_para[R120]=(Desire_vel_nomal*a)+380.0;
			curve_speed_para[R125]=(Desire_vel_nomal*a)+410.0;
			curve_speed_para[R130]=(Desire_vel_nomal*a)+440.0;
			curve_speed_para[R135]=(Desire_vel_nomal*a)+470.0;
			curve_speed_para[R140]=(Desire_vel_nomal*a)+500.0;
			curve_speed_para[R145]=(Desire_vel_nomal*a)+530.0;
			curve_speed_para[R150]=(Desire_vel_nomal*a)+560.0;
			curve_speed_para[R155]=(Desire_vel_nomal*a)+590.0;
			curve_speed_para[R160]=(Desire_vel_nomal*a)+620.0;
			curve_speed_para[R165]=(Desire_vel_nomal*a)+650.0;
			curve_speed_para[R170]=(Desire_vel_nomal*a)+680.0;
			curve_speed_para[R175]=(Desire_vel_nomal*a)+710.0;
			curve_speed_para[R180]=(Desire_vel_nomal*a)+740.0;
			curve_speed_para[R185]=(Desire_vel_nomal*a)+770.0;
			curve_speed_para[R190]=(Desire_vel_nomal*a)+800.0;
			curve_speed_para[R195]=(Desire_vel_nomal*a)+830.0;
			curve_speed_para[R200]=(Desire_vel_nomal*a)+860.0;	//2500mm/sくらい
			calc_speed(Desire_vel);
...
void calc_next_speed(float next_R)
{
	if(next_R < 150) 					next_max_speed=curve_speed_para[R10];
	if(next_R >= 150 && next_R < 200) 	next_max_speed=curve_speed_para[R15];
	if(next_R >= 200 && next_R < 250){	next_max_speed=curve_speed_para[R20];	LED_manager(10);}
	if(next_R >= 250 && next_R < 300) 	next_max_speed=curve_speed_para[R25];
	if(next_R >= 300 && next_R < 350) 	next_max_speed=curve_speed_para[R30];
	if(next_R >= 350 && next_R < 400) 	next_max_speed=curve_speed_para[R35];
	if(next_R >= 400 && next_R < 450) 	next_max_speed=curve_speed_para[R40];
	if(next_R >= 450 && next_R < 500) 	next_max_speed=curve_speed_para[R45];
	if(next_R >= 500 && next_R < 550){	next_max_speed=curve_speed_para[R50];	LED_manager(10);}
	if(next_R >= 550 && next_R < 600) 	next_max_speed=curve_speed_para[R55];
	if(next_R >= 600 && next_R < 650) 	next_max_speed=curve_speed_para[R60];
	if(next_R >= 650 && next_R < 700) 	next_max_speed=curve_speed_para[R65];
	if(next_R >= 700 && next_R < 750) 	next_max_speed=curve_speed_para[R70];
	if(next_R >= 750 && next_R < 800) 	next_max_speed=curve_speed_para[R75];
	if(next_R >= 800 && next_R < 850){	next_max_speed=curve_speed_para[R80];	LED_manager(10);}
	if(next_R >= 850 && next_R < 900) 	next_max_speed=curve_speed_para[R85];
	if(next_R >= 900 && next_R < 950) 	next_max_speed=curve_speed_para[R90];
	if(next_R >= 950 && next_R < 1000) 	next_max_speed=curve_speed_para[R95];
	if(next_R >= 1000 && next_R < 1050) 	next_max_speed=curve_speed_para[R100];
	if(next_R >= 1050 && next_R < 1100) 	next_max_speed=curve_speed_para[R105];
	if(next_R >= 1100 && next_R < 1150){	next_max_speed=curve_speed_para[R110];	LED_manager(10);}
	if(next_R >= 1150 && next_R < 1200) 	next_max_speed=curve_speed_para[R115];
	if(next_R >= 1200 && next_R < 1250) 	next_max_speed=curve_speed_para[R120];
	if(next_R >= 1250 && next_R < 1300) 	next_max_speed=curve_speed_para[R125];
	if(next_R >= 1300 && next_R < 1350) 	next_max_speed=curve_speed_para[R130];
	if(next_R >= 1350 && next_R < 1400) 	next_max_speed=curve_speed_para[R135];
	if(next_R >= 1400 && next_R < 1450){	next_max_speed=curve_speed_para[R140];	LED_manager(10);}
	if(next_R >= 1450 && next_R < 1500) 	next_max_speed=curve_speed_para[R145];
	if(next_R >= 1500 && next_R < 1550) 	next_max_speed=curve_speed_para[R150];
	if(next_R >= 1550 && next_R < 1600) 	next_max_speed=curve_speed_para[R160];
	if(next_R >= 1600 && next_R < 1650) 	next_max_speed=curve_speed_para[R165];
	if(next_R >= 1650 && next_R < 1700) 	next_max_speed=curve_speed_para[R170];
	if(next_R >= 1700 && next_R < 1750){	next_max_speed=curve_speed_para[R175];	LED_manager(10);}
	if(next_R >= 1750 && next_R < 1800) 	next_max_speed=curve_speed_para[R180];
	if(next_R >= 1800 && next_R < 1850) 	next_max_speed=curve_speed_para[R185];
	if(next_R >= 1850 && next_R < 1900) 	next_max_speed=curve_speed_para[R190];
	if(next_R >= 1900 && next_R < 1950) 	next_max_speed=curve_speed_para[R195];
	if(next_R >= 1950 && next_R <= 2000) 	next_max_speed=curve_speed_para[R200];
}
...
void trace2()	//二週目以降
{
	...
	sub_calc_accel_dis(Desire_vel_nomal);
	Accel_manager(0,abs(course_data[0]));
	now_max_speed=speed_para[now_comp];	
	calc_accel_dis(Desire_vel_nomal);
	next_R=abs(course_data[1]);
	calc_next_speed(next_R);
	init_ENC();
...
void init_thre(){
	...
	if(debug_mode_flag==0){
		...
		SEN_thre[corner]=160;//1200;//max_thre*0.3;//60;
		SEN_thre[corner2]=160;//1000;//max_thre*0.3;//60;
	}
