...
short get_gyro()	//ジャイロ値取得
{
	...
	AD0.ADCSR.BIT.CH=1;				//AD変換するアナログ入力チャンネルを設定  AD0.ADDRI
...
void search_max()
{
	...
	while((float)ENC_R*mmpp < 150.0){
		...
		MOT_ctr_L1=0;
		MOT_ctr_L2=1;
		MOT_R=100 - (0.1*(ENC_R-ENC_L));
		MOT_L=100 + (0.1*(ENC_R-ENC_L));
	...
	while((float)(40000-ENC_R)*mmpp < 150.0){
		MOT_R=100 - (0.001*(ENC_R-ENC_L));
		MOT_L=100 + (0.001*(ENC_R-ENC_L));
	...
	if(tmp > 2.0*accel_dis[0]){
		...
		if(tmp > 2.0*accel_dis[1]){
			...
			if((tmp > 2.0*accel_dis[2])){
				now_comp=2;
				if((tmp > 2.0*accel_dis[3])){
					now_comp=3;
					if((tmp > 2.0*accel_dis[4]) && (now_R > 3000)){
						now_comp=4;
					}
				}
			}
...
{
	if(ENC_dis[ENC_count+1]*mmpp > 350.0){
		if(next_R < 150) 					Desire_next_speed=curve_speed_para[R10];
		if(next_R >= 150 && next_R < 200) 	Desire_next_speed=curve_speed_para[R15];
		if(next_R >= 200 && next_R < 250){	Desire_next_speed=curve_speed_para[R20];	LED_manager(10);}
		if(next_R >= 250 && next_R < 300) 	Desire_next_speed=curve_speed_para[R25];
		if(next_R >= 300 && next_R < 350) 	Desire_next_speed=curve_speed_para[R30];
		if(next_R >= 350 && next_R < 400) 	Desire_next_speed=curve_speed_para[R35];
		if(next_R >= 400 && next_R < 450) 	Desire_next_speed=curve_speed_para[R40];
		if(next_R >= 450 && next_R < 500) 	Desire_next_speed=curve_speed_para[R45];
		if(next_R >= 500 && next_R < 550){	Desire_next_speed=curve_speed_para[R50];	LED_manager(10);}
		if(next_R >= 550 && next_R < 600) 	Desire_next_speed=curve_speed_para[R55];
		if(next_R >= 600 && next_R < 650) 	Desire_next_speed=curve_speed_para[R60];
		if(next_R >= 650 && next_R < 700) 	Desire_next_speed=curve_speed_para[R65];
		if(next_R >= 700 && next_R < 750) 	Desire_next_speed=curve_speed_para[R70];
		if(next_R >= 750 && next_R < 800) 	Desire_next_speed=curve_speed_para[R75];
		if(next_R >= 800 && next_R < 850){	Desire_next_speed=curve_speed_para[R80];	LED_manager(10);}
		if(next_R >= 850 && next_R < 900) 	Desire_next_speed=curve_speed_para[R85];
		if(next_R >= 900 && next_R < 950) 	Desire_next_speed=curve_speed_para[R90];
		if(next_R >= 950 && next_R < 1000) 	Desire_next_speed=curve_speed_para[R95];
		if(next_R >= 1000 && next_R < 1050) 	Desire_next_speed=curve_speed_para[R100];
		if(next_R >= 1050 && next_R < 1100) 	Desire_next_speed=curve_speed_para[R105];
		if(next_R >= 1100 && next_R < 1150){	Desire_next_speed=curve_speed_para[R110];	LED_manager(10);}
		if(next_R >= 1150 && next_R < 1200) 	Desire_next_speed=curve_speed_para[R115];
		if(next_R >= 1200 && next_R < 1250) 	Desire_next_speed=curve_speed_para[R120];
		if(next_R >= 1250 && next_R < 1300) 	Desire_next_speed=curve_speed_para[R125];
		if(next_R >= 1300 && next_R < 1350) 	Desire_next_speed=curve_speed_para[R130];
		if(next_R >= 1350 && next_R < 1400) 	Desire_next_speed=curve_speed_para[R135];
		if(next_R >= 1400 && next_R < 1450){	Desire_next_speed=curve_speed_para[R140];	LED_manager(10);}
		if(next_R >= 1450 && next_R < 1500) 	Desire_next_speed=curve_speed_para[R145];
		if(next_R >= 1500 && next_R < 1550) 	Desire_next_speed=curve_speed_para[R150];
		if(next_R >= 1550 && next_R < 1600) 	Desire_next_speed=curve_speed_para[R160];
		if(next_R >= 1600 && next_R < 1650) 	Desire_next_speed=curve_speed_para[R165];
		if(next_R >= 1650 && next_R < 1700) 	Desire_next_speed=curve_speed_para[R170];
		if(next_R >= 1700 && next_R < 1750){	Desire_next_speed=curve_speed_para[R175];	LED_manager(10);}
		if(next_R >= 1750 && next_R < 1800) 	Desire_next_speed=curve_speed_para[R180];
		if(next_R >= 1800 && next_R < 1850) 	Desire_next_speed=curve_speed_para[R185];
		if(next_R >= 1850 && next_R < 1900) 	Desire_next_speed=curve_speed_para[R190];
		if(next_R >= 1900 && next_R < 1950) 	Desire_next_speed=curve_speed_para[R195];
...
//		if(((ave_SEN[corner2]>SEN_thre[corner2])) && corner_frag==0 && goal_frag2==1 && (distance2 > 50.0)){			
			...
			sub_course_data[ENC_count]=sum_R/sub_count;
			sum_R=0;
			sub_count=0;
			ENC_count++;
		}
				...
				if(corner_frag==0){
					if(now_R > 1000){
						...
						if(next_R==0){	//最後の区画はぶっ飛ばせ
							...
							if(Desire_vel > speed_para[4])
								Desire_vel=speed_para[4];
							calc_speed(Desire_vel);	
						}		
...
//				Desire_gyro=Desire_vel/course_data[ENC_count];
		...
		/*********************ゴール処理*******************/				
		if((ave_SEN[goal] > SEN_thre[goal]) && goal_frag2==1 && course_data[ENC_count+2]==0 ){
			goal_count++;
			goal_frag2=0;
		}
		if(goal_count==circle_number){
...
void TIMER_CALL()
{
	...
	if(start0==1){								
		...
		now_gyro=(now_vel_R-now_vel_L)/Tred;		// rad/s
		//Desire_gyro=Desire_vel_nomal*now_dis/(100.0*Tred);	//350.0
		
		/***ゴールチェック***/
		switch(circl_count){
			case 1:	goal_check(now_ENC_R, now_ENC_L);
					/***角速度目標値更新***/
					if(now_dis < 50.0 || now_dis > -50.0){
						Desire_gyro=0.0;
					}
					else{
						Desire_gyro=now_gyro;
					}
					
					/***曲率計算(仮)***/
					sum_R+=(now_vel/now_gyro);
					sub_count++;
					break;

			case 2:	goal_check2(now_ENC_R, now_ENC_L, (short)ERR);
					if((course_data[ENC_count] > 1000) || (course_data[ENC_count] < -1000)){
						Desire_gyro=0.0;
					}
					else{
						Desire_gyro=Desire_vel/course_data[ENC_count];
...
{	
	...
	accel_dis[3]=((speed_para[3]*speed_para[3])-(Initial_velocity*Initial_velocity))/(2.0*Accel[3]);
	accel_dis[4]=((speed_para[4]*speed_para[4])-(Initial_velocity*Initial_velocity))/(2.0*Accel[4]);

	if(accel_dis[0] < 0.0) accel_dis[0]*=(-1.0);
	if(accel_dis[1] < 0.0) accel_dis[1]*=(-1.0);
	if(accel_dis[2] < 0.0) accel_dis[2]*=(-1.0);
	if(accel_dis[3] < 0.0) accel_dis[3]*=(-1.0);
	if(accel_dis[4] < 0.0) accel_dis[4]*=(-1.0);
}
...
void out_sens2()
{
	...
	while(1){
		dec_out(ave_SEN[6], 6);	outs(" ");	//corner1
		dec_out(ave_SEN[7], 6);	outs(" ");	//corner2
		dec_out(ave_SEN[8], 6);	outs(" "); outs(" ");	//goal

		dec_out(cari_SEN[6], 6);	outs(" ");	//corner1
		dec_out(cari_SEN[7], 6);	outs(" ");	//corner2
		dec_out(cari_SEN[8], 6);	outs(" ");	//goal
...
void out_course()
{
	...
	for(i=0; i<201; i++){
		...
		dec_out((short)(sub_course_data[i]), 6); outs(" ");
...
void init_thre(){
	...
	if(debug_mode_flag==0){
		SEN_thre[goal]  =150;//200;//2700;//max_thre*0.3;//TargetGoal*K_goal;//max_thre*0.3;//90;
		SEN_thre[corner]=150;//200;//160;//1200;//max_thre*0.3;//60;
