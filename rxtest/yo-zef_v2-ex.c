...
#define senc_dis1 35.0
#define senc_dis2 25.0
#define senc_dis3 15.0
#define senc_dis4 -15.0
#define senc_dis5 -25.0
#define senc_dis6 -35.0
...
//						K_D=10.1;
				...
				if(ENC_dis[counter] > 400.0 && ENC_dis[counter] < 1020.0){
					if(RL_check[counter]==right && N_E_R<(ENC_dis[counter] - 320.0)){
						Target_vel=speed_para[1];
//						K_D=10.1;
						K_G=first_K_G*1.5;
					}
					else if(RL_check[counter]==right && N_E_R>(ENC_dis[counter] - 320.0)){
						if(first_target_frag==0)
							Desire_vel=Target_vel;
					...
					if(RL_check[counter]==left && N_E_L<(ENC_dis[counter] - 320.0)){
						Target_vel=speed_para[1];
//						K_D=10.1;
						K_G=first_K_G*1.5;
					}		
					else if(RL_check[counter]==left && N_E_L>(ENC_dis[counter] - 320.0)){
						if(first_target_frag==0)
							Desire_vel=Target_vel;
				...
				if(ENC_dis[counter] > 1020.0){
					if(RL_check[counter]==right && N_E_R<(ENC_dis[counter] - 400.0)){
						Target_vel=speed_para[2];
//						K_D=10.1;
						K_G=first_K_G*1.5;
					}
					else if(RL_check[counter]==right && N_E_R>(ENC_dis[counter] - 400.0)){
						if(first_target_frag==0)
							Desire_vel=Target_vel;
					...
					if(RL_check[counter]==left && N_E_L<(ENC_dis[counter] - 400.0)){
						Target_vel=speed_para[2];
//						K_D=10.1;
						K_G=first_K_G*1.5;
					}
					else if(RL_check[counter]==left && N_E_L>(ENC_dis[counter] - 400.0)){
						if(first_target_frag==0)
							Desire_vel=Target_vel;
...
short goal_check(long RIGHT_ENC, long LEFT_ENC)
{
	...
	if(first_check==2){		
		if((ave_SEN[1] > thre_max) && 
			(ave_SEN[6] > thre_max) && goal_frag2==1){
			goal_frag2=0;
		}
...
void record_log(float now_dis, long E_R, long E_L)
{
	long 
	if(now_dis > 100.0){
		angl_vel=E_R-E_L;		
		...
		curvature=(E_R+E_L)*Tred/(2*angl_vel);
		if(curvature<0)
			curvature*=(-1);

		ENC_dis[ENC_count]=now_dis;
		ENC_check[ENC_count]=(short)curvature/10;
		ENC_count++;
		now_ENC_R=0;
		now_ENC_L=0;	
...
void TIMER_CALL()
{
	...
	if(start0==1){				
		...
		record_log((float)ave_ENC*mmpp, now_ENC_R, now_ENC_L);		
		calc_senc();					
...
void Battery_Check()
{
	...
	if(battery < 757 && battery >= 650){
		LED_manager(3);	
	}
	if(battery < 650){								//7.4V未満になったら未満強制停止
		LED_manager(4);
	//	while(1){
			//mot_STB(STOP);
	//	}
...
//	select_corner();
	...
	while(trace_roop_frag==0){
		...
		if(cari_SEN[6] > 50.0 && permit_goal==1){
			start_frag=1;
			LED3=ON;
			TIMER_WAIT(500);
		}
		else if(cari_SEN[6] > 50.0 && permit_goal==0){	//ゴールさせない
			LED3=ON;
			//Dispermit_goal=1;
		}	
...
void line_check()
{
	...
	while(1){
		...
		if(max_frag==1 && (ave_SEN[1] < 30.0 && ave_SEN[2] < 30.0 && ave_SEN[3] < 30.0 && ave_SEN[4] < 30.0 && ave_SEN[5] < 30.0 && ave_SEN[6] < 30.0)){
			LED1=OFF;
			ERR=max_ERR;
		}
				
		if(min_frag==1 && (ave_SEN[1] < 30.0 && ave_SEN[2] < 30.0 && ave_SEN[3] < 30.0 && ave_SEN[4] < 30.0 && ave_SEN[5] < 30.0 && ave_SEN[6] < 30.0)){
			ERR=min_ERR;
			LED1=OFF;
		}
...
int init_thre(){
	...
	SEN_thre[goal]  =45.0;//max_thre*0.3;//TargetGoal*K_goal;//max_thre*0.3;//90;
	SEN_thre[corner]=40.0;//max_thre*0.3;//60;
