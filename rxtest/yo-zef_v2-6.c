...
double ave_SEN[11];
double dis_SEN[11];
double sum_dis_SEN[11];
double MAX_dis_SEN[11];
double MIN_dis_SEN[11];
double ave_dis_SEN[11];
double SEN_min[11];
double SEN_max[11];
double SEN_thre[11];
...
short goal_check(long RIGHT_ENC, long LEFT_ENC)
{
	...
	if(first_check==0){
		if(cari_SEN[goal] > SEN_thre[goal]){						//スタート時 白線上
			goal_frag1=1;
			LED3=ON;
		}
	
		if(goal_frag1==1 && (cari_SEN[goal] < (SEN_thre[goal]))){		//スタート時 白線から外れたとき
			LED3=OFF;
			goal_frag2=1;
			first_check=1;
			StartTimeCount=0;
	...
	if(first_check==2){		
		if((ave_SEN[1] > thre_max2) && 
			(ave_SEN[6] > thre_max2) && goal_frag2==1){
			goal_frag2=0;
		}
...
int init_thre(){
	...
	SEN_thre[goal]  =30.0;//max_thre*K_goal;//max_thre*0.3;//TargetGoal*K_goal;//max_thre*0.3;//90;
