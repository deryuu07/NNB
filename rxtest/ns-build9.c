...
{
	float tmp = ENC_dis[ENC_count+1]*mmpp;
	...
	if(next_R < 1000){
		...
		if(tmp > 300.0){
			if(tmp > 2.0*accel_dis[0]){		//直線速度パラメータ決定
				now_comp=0;
				if(tmp > 2.0*accel_dis[1]){
					now_comp=1;
					if((tmp > 2.0*accel_dis[2]))
			...
			Desire_next_speed=speed_para[now_comp];		
		}
		else{
			Desire_next_speed=curve_speed_para[R10];
		}
	...
	next_top_speed=Desire_next_speed;
}
...
void goal_check2(long RIGHT_ENC, long LEFT_ENC, short err)	//カーブ判定
{
	...
	if(first_check==2){		
		...
		//---加減速のアルゴリズム---
		//最初の区画は加速
		//if 現区画のトップスピードより次区画のトップスピードのほうが速い
		//	現在の速度を維持
		//else 
		//  次区画の速度まで減速
		//
		...
		if(accel_safety==0){
			if(goal_frag1==1){	
				...
				if(((ave_SEN[corner2]>SEN_thre[corner2]) || (ave_SEN[corner]>SEN_thre[corner])) && distance2 > (now_comp_distance-50.0) && corner_frag==0 && goal_frag2==1){
					...
					accel_distance=((now_top_speed*now_top_speed)-(first_speed*first_speed))/(2.0*Accel[0]);
...
{	
	...
	accel_dis[1]=((speed_para[1]*speed_para[1])-(Initial_velocity*Initial_velocity))/(2.0*Accel[0]);
	accel_dis[2]=((speed_para[2]*speed_para[2])-(Initial_velocity*Initial_velocity))/(2.0*Accel[0]);
...
void trace2()	//二週目以降
{
	...
	calc_next_speed(abs(course_data[1]));
	now_top_speed=speed_para[now_comp];
	accel_distance=((now_top_speed*now_top_speed)-(Desire_vel_nomal*Desire_vel_nomal))/(2.0*Accel[0]);
	if(accel_distance < 0.0) accel_distance*=(-1.0);
